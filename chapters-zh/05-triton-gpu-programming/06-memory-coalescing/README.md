...
# 第 06 课 — 用 stride 诊断合并访存

> **问题：**当offset 几何、transaction 与请求带宽同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/06-memory-coalescing/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/06-memory-coalescing/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/06-memory-coalescing/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**offset 几何、transaction 与请求带宽**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
合并访存取决于相邻 lane 请求的地址，而不是 tensor 看起来的形状。同一份 row-copy 源码只需改变 column stride，就会产生连续或跨步地址。请求带宽必须与
stride 和字节公式一起解释。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["offset 几何、transaction 与请求带宽"]
...
**实验：**让同一个 stride-aware Triton copy 分别处理连续列和 stride=2 的 view。
...
|---|---:|
| 连续布局中位延迟 | 0.0173 ms |
| stride=2 中位延迟 | 0.0178 ms |
| 最大绝对误差 | 0.000e+00 |
| 验收 gate | true |

### 如何解释结果

同一个 copy kernel 在连续列上用时 0.0173 ms，在 stride=2 时用时 0.0178 ms，二者相差 1.03 倍。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/06-memory-coalescing/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。在尚未证明访存问题前就加入 shared
memory，可能增加工作量，却没有修正真正的地址模式。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 6 --end 6
