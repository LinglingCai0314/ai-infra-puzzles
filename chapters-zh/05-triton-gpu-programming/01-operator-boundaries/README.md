...
# 第 01 课 — 算子边界与小 kernel 的真实成本

> **问题：**当融合、launch 数与显存流量同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/01-operator-boundaries/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/01-operator-boundaries/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/01-operator-boundaries/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**融合、launch 数与显存流量**。目的不是把每个 PyTorch 操作都改成自定义代码， 而是把一个性能判断缩小到可以逐项检查：正确性、计时、布局、编译状态和对照路径都要
...
eager 表达式可能在每个算子边界都完整读写 tensor。Triton program 可以让中间值停留在寄存器中，只写回一次。因此实验固定代数表达式，并把 launch
数、请求字节与延迟分开。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["融合、launch 数与显存流量"]
...
**实验：**在同一 tensor 上，对比一个 Triton affine kernel 与等价的两步 eager PyTorch CUDA 路径。
...
| PyTorch eager 中位延迟 | 0.0189 ms |
| 最大绝对误差 | 4.768e-07 |
| 验收 gate | true |

### 如何解释结果

融合 Triton affine kernel 用时 0.0211 ms；两个 eager PyTorch 操作合计 0.0189 ms，最大误差为 4.77e-07。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/01-operator-boundaries/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。融合 kernel
更快，不代表所有算子都该重写。输入很小、编译器已融合，或库函数足够强时，优势可能消失。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
