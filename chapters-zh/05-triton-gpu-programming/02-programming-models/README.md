...
# 第 02 课 — Blocked program 与 CUDA thread

> **问题：**当program_id、blocked tensor 与标量 thread同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/02-programming-models/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/02-programming-models/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/02-programming-models/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**program_id、blocked tensor 与标量 thread**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
CUDA 暴露的是按 block 组织的标量 thread；Triton 暴露 program instance 的 grid，每个 instance 计算 tl.arange 一类
tensor 表达式。它可以近似类比一个 CUDA block，但不能把二者视为完全等同。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["program_id、blocked tensor 与标量 thread"]
...
**实验：**让每个 Triton program 处理 256 个元素，并与 torch.add 对照。
...
| 测量字段 | 仓库记录值 |
|---|---:|
| Triton 中位延迟 | 0.0211 ms |
| torch.add 中位延迟 | 0.0136 ms |
| 最大绝对误差 | 0.000e+00 |
| 验收 gate | true |

### 如何解释结果

16,384 个 blocked program 覆盖 4,194,304 个标量元素；结果与 torch.add 一致，两条路径分别计时。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/02-programming-models/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。把 blocked tensor 的每个 lane 当成独立调度的
CUDA thread，会对同步与存储做出错误假设。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 2 --end 2
