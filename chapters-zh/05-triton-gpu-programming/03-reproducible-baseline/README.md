...
> **问题：**当JIT cache、目标身份与冷暖时间同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/03-reproducible-baseline/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/03-reproducible-baseline/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/03-reproducible-baseline/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**JIT cache、目标身份与冷暖时间**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
Triton 性能同时受 Python package、编译流水线、backend target、driver/runtime、输入特化和 cache 状态影响。只记录 GPU
名称不构成实验身份；冷启动主机时间与 warm event 时间必须分列。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["JIT cache、目标身份与冷暖时间"]
...
**实验：**记录完整环境，并把首次主机观察调用与 warm GPU-event 样本分开测量。
...
|---|---:|
| 首次主机调用 | 330.9141 ms |
| Warm GPU 中位延迟 | 0.0206 ms |
| 最大绝对误差 | 4.768e-07 |
| 验收 gate | true |

### 如何解释结果

主机观察到的首次调用为 330.91 ms，warm GPU-event 中位数为 0.0206 ms；二者回答的是不同问题。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/03-reproducible-baseline/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。nvidia-smi 显示的 CUDA Version 是 driver
capability 信号，不等于本地 Toolkit 或 kernel 编译器版本。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
