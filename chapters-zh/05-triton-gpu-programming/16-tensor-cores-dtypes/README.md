...
# 第 16 课 — Tensor Core 与 dtype 语义

> **问题：**当输入 dtype、累加精度、吞吐与误差同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/16-tensor-cores-dtypes/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/16-tensor-cores-dtypes/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/16-tensor-cores-dtypes/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**输入 dtype、累加精度、吞吐与误差**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
当 dtype、shape 与 backend 允许时，tl.dot 可以降低到目标矩阵指令。输入存储、乘法精度、累加类型和输出 cast
是不同选择；吞吐表若没有相对明确参考的误差，就不完整。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["输入 dtype、累加精度、吞吐与误差"]
...
**实验：**对比 BF16 Triton 与 torch.mm，保留 dtype、延迟、吞吐和相对 FP32 的误差。
...
| 测量字段 | 仓库记录值 |
|---|---:|
| Triton BF16 中位延迟 | 0.0251 ms |
| 库 BF16 中位延迟 | 0.0155 ms |
| 最大绝对误差 | 0.000e+00 |
| 验收 gate | true |

### 如何解释结果

BF16 Triton 与库 GEMM 分别为 0.0251 和 0.0155 ms；按 FP32 比较的最大误差为 0.000e+00。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/16-tensor-cores-dtypes/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。把 FP16、BF16、TF32 与 FP8 合并成一行“Tensor
Core”结果，实际上混淆了不同数值 contract。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 16 --end 16
