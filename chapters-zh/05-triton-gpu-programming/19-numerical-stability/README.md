...
# 第 19 课 — 数值稳定性与 cast 边界

> **问题：**当max shift、FP32 累加、overflow 与 underflow同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/19-numerical-stability/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/19-numerical-stability/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/19-numerical-stability/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**max shift、FP32 累加、overflow 与 underflow**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["max shift、FP32 累加、overflow 与 underflow"]
...
**实验：**向未 shift 的 eager Softmax 和 max-shift Triton Softmax 输入大幅正 FP16 logits。
...
未减最大值的 FP16 表达式产生 2,096,519 个非有限值；max-shift Triton softmax 没有非有限值，相对 FP32 参考最大误差 2.439e-04。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/19-numerical-stability/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。小范围随机输入可能永远触发不了
overflow、underflow、抵消或长归约误差。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 19 --end 19
