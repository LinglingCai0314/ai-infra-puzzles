...
# 第 23 课 — Paged KV Cache 寻址

> **问题：**当block table、逻辑 token、物理 block 与 gather 布局同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/23-paged-kv-gather/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/23-paged-kv-gather/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/23-paged-kv-gather/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**block table、逻辑 token、物理 block 与 gather 布局**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
Paged attention 把 token 的逻辑位置与物理 KV block 分离。block table lookup 选择物理块，取模选择块内 slot；kernel
既要保持间接寻址，又要保住 width 方向的合并访存。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["block table、逻辑 token、物理 block 与 gather 布局"]
...
**实验：**在随机逻辑到物理 block table 上实现 Triton paged gather，并与 advanced indexing 对照。
...
| 测量字段 | 仓库记录值 |
|---|---:|
| Triton gather 中位延迟 | 0.0171 ms |
| PyTorch indexing 中位延迟 | 0.0390 ms |
| 最大绝对误差 | 0.000e+00 |
| 验收 gate | true |

### 如何解释结果

Triton paged gather 按 128 个逻辑到物理 block 映射完成读取，用时 0.0171 ms，与 advanced indexing 完全一致。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/23-paged-kv-gather/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。连续 cache 上的正确性不能覆盖 block-table
permutation、最后一个不完整 block 或无效 slot ownership。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 23 --end 23
