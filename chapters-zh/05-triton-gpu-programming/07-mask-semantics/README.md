...
# 第 07 课 — mask 与 reduction 中性元

> **问题：**当masked load、other 值与归约代数同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/07-mask-semantics/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/07-mask-semantics/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/07-mask-semantics/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**masked load、other 值与归约代数**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["masked load、other 值与归约代数"]
...
**实验：**在 1,024 行全负输入上，对比正确的 -inf padding 与错误的 0 padding。
...
max reduction 的 mask 使用 -inf 时误差为 0.00e+00；错误地填 0 会破坏 1024 行全负输入。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/07-mask-semantics/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。全负数 max reduction 若用 0
padding，会悄悄返回看似合理却不可能的结果。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 7 --end 7
