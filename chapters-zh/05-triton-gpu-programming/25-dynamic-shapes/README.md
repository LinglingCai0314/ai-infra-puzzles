...
> **问题：**当运行期参数、constexpr meta-parameter、tail 与 cache key同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/25-dynamic-shapes/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/25-dynamic-shapes/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/25-dynamic-shapes/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**运行期参数、constexpr meta-parameter、tail 与 cache key**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
只用于 mask 与 grid 计算的 size 可以保留为运行期参数，让同一 compiled kernel 覆盖多种长度；决定 tl.arange 形状或编译期分支的值必须是
constexpr，并形成特化边界。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["运行期参数、constexpr meta-parameter、tail 与 cache key"]
...
**实验：**让同一份 affine 源码处理三种长度和不同 tail，并保留每种 shape 的 first/warm 结果。
...
|---|---:|
| 通过的 shape 数 | 3 |
| 最大 warm 中位延迟 | 0.0193 ms |
| 最大绝对误差 | 4.768e-07 |
| 验收 gate | true |

### 如何解释结果

同一个 runtime-size kernel 在不改源码的情况下处理了 3 个 shape 和三种 tail；最大 warm 中位数为 0.0193 ms。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/25-dynamic-shapes/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。把所有 shape 都标成 constexpr
会造成编译抖动；把所有选择都做成动态，又可能阻止有效优化。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 25 --end 25
