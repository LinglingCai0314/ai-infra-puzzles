...
# 第 29 课 — Triton 与 CUDA 选型框架

> **问题：**当性能、shape 稳定性、团队能力、生命周期、平台与验证同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/29-selection-framework/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/29-selection-framework/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/29-selection-framework/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**性能、shape 稳定性、团队能力、生命周期、平台与验证**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["性能、shape 稳定性、团队能力、生命周期、平台与验证"]
...
**实验：**把一次 affine 实测与权重可见的场景 scorecard 结合起来。
...
| 测量字段 | 仓库记录值 |
|---|---:|
| Triton/PyTorch 实测比值 | 1.107x |
| Pointwise 场景选择 | triton |
| 最大绝对误差 | 0.000e+00 |
| 验收 gate | true |

### 如何解释结果

affine 实测比值为 1.107；显式决策矩阵在不同约束下选出不同工具，没有写入“万能赢家”。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/29-selection-framework/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。把 Triton 宣称为 CUDA 的完全替代，或把 CUDA
当成唯一严肃选择，都会忽略眼前算子与团队的真实约束。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
**证据标签：**[`capacity-model`](../README.md#证据标签)。实测值进入显式决策或流量模型。模型用于组织证据，不能替代目标 backend 执行。
