...
# 第 05 课 — 显式 CUDA 控制与错误边界

> **问题：**当thread block、同步、nvcc 与可审阅源码同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/05-explicit-cuda-control/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/05-explicit-cuda-control/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/05-explicit-cuda-control/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**thread block、同步、nvcc 与可审阅源码**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
CUDA C++ 显式写出标量 thread index、block 几何、launch、同步和错误检查；Triton 把其中许多选择交给 blocked program
与编译器。公平对照既需要两份实现，也需要真正能构建 CUDA 路径的工具链。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["thread block、同步、nvcc 与可审阅源码"]
...
**实验：**运行 Triton 实现，检查 nvcc 可用性，并保留带 launch check 的等价 CUDA C++ 源码。

| 实验角色 | 固定定义 |
|---|---|
| Baseline | 可审阅 CUDA C++ 源码与工具链可用性 |
...
|---|---:|
| nvcc 可用 | false |
| Triton 中位延迟 | 0.0212 ms |
| 最大绝对误差 | 4.768e-07 |
| 验收 gate | true |

### 如何解释结果

Triton 路径正确运行，用时 0.0212 ms。nvcc 可用性为 False，因此显式 CUDA 源码可审阅，但不冒充原生实测。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/05-explicit-cuda-control/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。有源码不等于完成原生执行。没有 nvcc 时，CUDA latency
必须明确保留为未测。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
