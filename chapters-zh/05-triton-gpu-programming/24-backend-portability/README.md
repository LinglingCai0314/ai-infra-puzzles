...
# 第 24 课 — Backend 可移植性与 ROCm 边界

> **问题：**当共享源码、backend target、架构调优与未测平台同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/24-backend-portability/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/24-backend-portability/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/24-backend-portability/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**共享源码、backend target、架构调优与未测平台**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["共享源码、backend target、架构调优与未测平台"]
...
**实验：**记录 active Triton backend 与 architecture，并明确保留同源码 ROCm 路径为未执行。
...
本次将同一份 Triton 源码编译到 backend=cuda、arch=120；由于只运行了 NVIDIA 环境，ROCm 可移植性仍未实测。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/24-backend-portability/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。kernel 在 NVIDIA 上编译成功，即使没有
CUDA-specific call，也不能当作 ROCm 性能证据。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 24 --end 24
