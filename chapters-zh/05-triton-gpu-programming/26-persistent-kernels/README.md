...
# 第 26 课 — Persistent 调度与 TMA 边界

> **问题：**当固定 resident program、tile loop、launch 几何与架构特性同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/26-persistent-kernels/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/26-persistent-kernels/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/26-persistent-kernels/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**固定 resident program、tile loop、launch 几何与架构特性**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
persistent kernel 启动较少 program，让每个 program 循环处理多个 tile。它可能摊薄调度成本或支持 producer-consumer
pipeline，也可能降低灵活性与公平性。TMA 是独立的架构相关数据搬运机制，不等于 persistence。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["固定 resident program、tile loop、launch 几何与架构特性"]
...
**实验：**对比 256-program 的 persistent affine grid 与每 tile 一个 program 的标准 grid；本实验不使用 TMA。
...
| 测量字段 | 仓库记录值 |
|---|---:|
| Persistent 中位延迟 | 0.0216 ms |
| 标准 grid 中位延迟 | 0.0202 ms |
| 最大绝对误差 | 4.768e-07 |
| 验收 gate | true |

### 如何解释结果

persistent 调度把 grid 从 16,384 个 program 降到 256 个，用时 0.0216 ms；标准 grid 为 0.0202 ms。本实验未使用 TMA。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/26-persistent-kernels/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。没有 descriptor、受支持的 load/store 路径或
profiler 证据，却把 tile loop 标成“TMA 优化”，属于过度结论。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 26 --end 26
