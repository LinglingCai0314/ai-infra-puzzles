...
# 第 17 课 — 融合 LayerNorm 与 RMSNorm

> **问题：**当逐行统计、FP32 累加与 affine 输出同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/17-fused-rmsnorm/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/17-fused-rmsnorm/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/17-fused-rmsnorm/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**逐行统计、FP32 累加与 affine 输出**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
RMSNorm 读取一行，用 FP32 累加平方和，计算 reciprocal RMS，乘学习权重并一次写回；LayerNorm 还要减均值。二者频繁且对访存敏感，hidden width
决定单 program 的实际边界。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["逐行统计、FP32 累加与 affine 输出"]
...
**实验：**实现融合 RMSNorm forward，并与等价 eager PyTorch 表达式对照。
...
| 测量字段 | 仓库记录值 |
|---|---:|
| Triton 中位延迟 | 0.0173 ms |
| PyTorch eager 中位延迟 | 0.0445 ms |
| 最大绝对误差 | 1.907e-06 |
| 验收 gate | true |

### 如何解释结果

融合 RMSNorm forward 为 0.0173 ms，eager 表达式为 0.0445 ms，最大误差 1.91e-06；本实验不覆盖 backward。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/17-fused-rmsnorm/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。只测 forward 不能代表
backward；输入梯度与权重梯度可能需要额外归约和同步。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
