...
# 第 18 课 — 在线 Softmax 与融合 Attention

> **问题：**当score 分块、运行中归一化量与避免物化同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/18-online-softmax-attention/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/18-online-softmax-attention/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/18-online-softmax-attention/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**score 分块、运行中归一化量与避免物化**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
融合 Attention 会分块计算 QK，更新 online Softmax 归一化量，并立即在 PV 中消费概率。核心不变量是完整 score matrix 不必驻留 HBM；具体实现则随
backend、head dimension、causal mask 与硬件代际变化。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["score 分块、运行中归一化量与避免物化"]
...
**实验：**对比 PyTorch causal SDPA 与显式物化的 eager attention，并计算 score tensor 字节数。
...
|---|---:|
| SDPA 中位延迟 | 0.0262 ms |
| 物化 eager 中位延迟 | 0.0777 ms |
| 最大绝对误差 | 1.953e-03 |
| 验收 gate | true |

### 如何解释结果

PyTorch SDPA 用时 0.0262 ms，显式物化 attention 为 0.0777 ms。算法边界是避免 8.0 MiB score tensor；不臆测 SDPA 内部
kernel 名称。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/18-online-softmax-attention/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。FlashAttention 是算法族，不能据此断言每个 SDPA
调用都使用某个固定 kernel。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 18 --end 18
