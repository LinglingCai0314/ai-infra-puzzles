...
# 第 12 课 — Reduction 与 Scan

> **问题：**当树形归约、前缀依赖与中性元同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/12-reduction-and-scan/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/12-reduction-and-scan/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/12-reduction-and-scan/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**树形归约、前缀依赖与中性元**。目的不是把每个 PyTorch 操作都改成自定义代码， 而是把一个性能判断缩小到可以逐项检查：正确性、计时、布局、编译状态和对照路径都要
...
Reduction 把一行折叠成一个值，scan 则产生每个前缀。Triton 都用 blocked tensor operation 表达，但依赖图与输出流量不同；tail padding
仍需正确中性元。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["树形归约、前缀依赖与中性元"]
...
**实验：**在同一个 4096×256 tensor 上运行 row sum 与 inclusive cumsum，并验证两份输出。
...
逐行 reduction 用时 0.0162 ms，inclusive scan 用时 0.0155 ms；即使输入 shape 相同，通信结构也不同。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/12-reduction-and-scan/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。超大行若全部留在一个 program
中，即使源码很短，也可能增加寄存器压力并减少 active work。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 12 --end 12
