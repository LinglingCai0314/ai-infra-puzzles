...
# 第 15 课 — 寄存器、warp 与 occupancy 权衡

> **问题：**当BLOCK、num_warps、live value 与延迟同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/15-resources-occupancy/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/15-resources-occupancy/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/15-resources-occupancy/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**BLOCK、num_warps、live value 与延迟**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
BLOCK 与 num_warps 会影响每个 program 的工作量、并行发射、live value 和 resident program 数。受控配置 sweep
能揭示敏感性，但不能直接给出寄存器或 achieved occupancy。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["BLOCK、num_warps、live value 与延迟"]
...
**实验：**对同一个 affine kernel sweep 四组 BLOCK/warp，并保留全部延迟样本。
...
实测最佳配置为 b1024-w8，最慢与最佳的延迟比为 1.70 倍；实验没有仅凭计时反推 occupancy。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/15-resources-occupancy/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。追求理论最大 occupancy 可能迫使 tile
变小或增加重复流量，反而降低性能。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
