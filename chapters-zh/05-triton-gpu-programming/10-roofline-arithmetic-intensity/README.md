...
# 第 10 课 — 调优前先做 Roofline 判断

> **问题：**当FLOP、HBM 字节、算术强度与上界同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/10-roofline-arithmetic-intensity/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/10-roofline-arithmetic-intensity/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/10-roofline-arithmetic-intensity/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**FLOP、HBM 字节、算术强度与上界**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
算术强度用有效操作数除以外部显存字节。低强度 pointwise kernel 通常要减少往返显存，高强度矩阵乘则要提高 Tensor Core tiling
效率。这个模型用于选择下一问，不能独自定位实测硬件瓶颈。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["FLOP、HBM 字节、算术强度与上界"]
...
**实验：**显式计算 affine kernel 与方阵 GEMM 的算法算术强度，再使用各自相关的单位测量。
...
affine kernel 的请求算术强度为 0.250 FLOP/byte，而方阵 GEMM 模型为 170.7；两者不能用同一吞吐指标判断。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/10-roofline-arithmetic-intensity/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。用 TFLOP/s 描述 memory-bound copy，或用
GB/s 描述 compute-bound GEMM，都会掩盖真正上界。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 10 --end 10
