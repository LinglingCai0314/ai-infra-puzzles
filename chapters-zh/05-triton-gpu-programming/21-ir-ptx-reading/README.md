...
# 第 21 课 — 从 Triton 源码到 IR 与 PTX

> **问题：**当TTIR、TTGIR、LLVM IR、PTX 与 profiler 对照同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/21-ir-ptx-reading/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/21-ir-ptx-reading/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/21-ir-ptx-reading/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**TTIR、TTGIR、LLVM IR、PTX 与 profiler 对照**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
Triton function 会经过高层 tensor IR、target-aware GPU IR、LLVM IR 和 backend
assembly。阅读这些层可以核查向量化、mask、load 与 dot lowering；性能因果仍需要运行样本和硬件 counter。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["TTIR、TTGIR、LLVM IR、PTX 与 profiler 对照"]
...
**实验：**检查 affine 源码、target 身份、mask 信号，以及文档化 dump/profiler 路径。
...
|---|---:|
| 源码行数 | 5 |
| Lowering 路径 | TTIR -> TTGIR -> LLVM IR -> PTX |
| 最大绝对误差 | 4.768e-07 |
| 验收 gate | true |

### 如何解释结果

被审阅的 kernel 源码有 5 行，目标为 GPUTarget(backend='cuda', arch=120, warp_size=32)。阅读 IR/PTX 是取证路径，不能代替
counter。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/21-ir-ptx-reading/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。看到一个预期 PTX mnemonic，不能证明 kernel
高效，也不能证明它支配应用时间。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
