...
> **问题：**当golden output、样本分布、版本锁定与 warning/error gate同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/28-performance-regression-ci/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/28-performance-regression-ci/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/28-performance-regression-ci/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**golden output、样本分布、版本锁定与 warning/error gate**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
kernel CI 同时需要正确性与性能 contract。正确性使用极端 shape 与 golden reference；性能在受控硬件上比较稳定样本摘要。版本身份必须跟随
baseline，因为编译器与 backend 变化会改变生成代码。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["golden output、样本分布、版本锁定与 warning/error gate"]
...
**实验：**收集两组独立的 30-sample，并按固定 warning/error gate 对中位数比值分类。
...
两组独立 warm 样本得到 candidate/baseline=0.975，按固定的 1.10/1.20 warning/error gate 判为 pass。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/28-performance-regression-ci/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。单个噪声样本，或只留在开发者电脑上的
benchmark，都不能支持可审计的回归判断。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
