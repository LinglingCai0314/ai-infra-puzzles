...
# 第 14 课 — Autotune 搜索与实验预算

> **问题：**当配置集合、cache key 与搜索成本同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/14-autotune-budget/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/14-autotune-budget/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/14-autotune-budget/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**配置集合、cache key 与搜索成本**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["配置集合、cache key 与搜索成本"]
...
**实验：**搜索三组 affine BLOCK/warp 配置，并分开记录首次主机成本与缓存后的 warm 延迟。
...
共有 3 个候选配置；首次主机调用为 570.64 ms，缓存后的 warm 中位数为 0.0333 ms。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/14-autotune-budget/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。会累加或原地修改输出的 autotune kernel 可能需要
reset hook，因为搜索阶段每个候选都会执行。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 14 --end 14
