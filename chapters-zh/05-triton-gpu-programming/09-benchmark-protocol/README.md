...
# 第 09 课 — Benchmark 协议与计时误差

> **问题：**当warmup、同步、分位数与 JIT同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/09-benchmark-protocol/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/09-benchmark-protocol/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/09-benchmark-protocol/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**warmup、同步、分位数与 JIT**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["warmup、同步、分位数与 JIT"]
...
**实验：**测量一次首次主机调用，再保留 40 个 CUDA-event 样本及 p20/p50/p80。
...
首次主机调用为 349.09 ms；warm event 计时为 p20=0.0190、p50=0.0201、p80=0.0213 ms。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/09-benchmark-protocol/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。把首次 JIT
调用混入稳态平均值，可能完全支配结果并反转决策。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
