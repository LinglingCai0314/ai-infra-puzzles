...
# 第 27 课 — CUTLASS、cuBLAS、cuDNN 还是 Triton

> **问题：**当库覆盖、epilogue 融合、可维护性与峰值调优同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/27-library-or-custom/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/27-library-or-custom/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/27-library-or-custom/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**库覆盖、epilogue 融合、可维护性与峰值调优**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
cuBLAS 与 cuDNN 提供高度调优的标准操作，CUTLASS 用 C++ template 构建架构感知 kernel，Triton 则以紧凑 DSL 表达自定义 blocked
dataflow。选择取决于操作覆盖、融合价值、平台范围、峰值目标与维护预算。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["库覆盖、epilogue 融合、可维护性与峰值调优"]
...
**实验：**把 ReLU 融合进教学版 Triton GEMM store，并与 torch.mm 加 ReLU 对照。
...
| 测量字段 | 仓库记录值 |
|---|---:|
| 融合 Triton 中位延迟 | 0.0229 ms |
| 库加 ReLU 中位延迟 | 0.0194 ms |
| 最大绝对误差 | 0.000e+00 |
| 验收 gate | true |

### 如何解释结果

自定义路径把 ReLU 融合进 GEMM store，用时 0.0229 ms；torch.mm 加 ReLU 为 0.0194 ms。这只是一个 shape，不是库排名。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/27-library-or-custom/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。自定义 GEMM 即使赢了一个
shape，若失去库覆盖、测试与未来架构支持，整体仍可能倒退。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
