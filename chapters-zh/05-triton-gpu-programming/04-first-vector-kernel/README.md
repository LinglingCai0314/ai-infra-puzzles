...
# 第 04 课 — 第一个 tail-safe 向量 kernel

> **问题：**当program_id、tl.arange、mask 与 BLOCK同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/04-first-vector-kernel/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/04-first-vector-kernel/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/04-first-vector-kernel/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**program_id、tl.arange、mask 与 BLOCK**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
向量 kernel 用 program_id 和编译期 BLOCK 推导 offset；运行期元素数控制 tail mask，BLOCK 则决定编译器看到的 tensor
形状。不能整除的长度也是 kernel contract 的一部分。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["program_id、tl.arange、mask 与 BLOCK"]
...
**实验：**在奇数长度向量上 sweep 四个 BLOCK，并验证全部 tail 元素。
...
非整除 tail 保持正确；四点 sweep 中 BLOCK=512 最快，中位数 0.0187 ms。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/04-first-vector-kernel/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。省略 load/store mask
可能在对齐样例上看似正确，却会在第一个非对齐 shape 上越界。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
