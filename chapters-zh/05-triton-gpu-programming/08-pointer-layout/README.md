...
# 第 08 课 — 指针算术与 tensor 布局

> **问题：**当逻辑索引、物理 stride 与转置 view同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/08-pointer-layout/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/08-pointer-layout/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/08-pointer-layout/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**逻辑索引、物理 stride 与转置 view**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["逻辑索引、物理 stride 与转置 view"]
...
**实验：**使用显式行列 stride，把转置后的 non-contiguous view 复制到连续输出。
...
显式 stride 算术正确复制了 2048x769 的转置 view，请求带宽为 461.3 GB/s。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/08-pointer-layout/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。只测 contiguous
tensor，可能让偶然正确的指针公式漏过验证。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
