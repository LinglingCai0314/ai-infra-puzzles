...
# 第 13 课 — Matmul tiling 与库函数边界

> **问题：**当block tile、tl.dot、L2 复用与 cuBLAS 基线同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/13-matmul-tiling/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/13-matmul-tiling/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/13-matmul-tiling/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**block tile、tl.dot、L2 复用与 cuBLAS 基线**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
Triton GEMM 让每个 program 负责一个输出 tile，沿 K 维推进、用 FP32 累加并一次写回。tile 大小共同影响 Tensor Core、复用与
occupancy。标准 GEMM 是厂商库的强项，因此库函数必须保留为 control。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["block tile、tl.dot、L2 复用与 cuBLAS 基线"]
...
**实验：**在固定的 512 方阵上，对比精简教学版 Triton FP16 GEMM 与 torch.mm。
...
| 测量字段 | 仓库记录值 |
|---|---:|
| Triton 中位延迟 | 0.0229 ms |
| torch.mm 中位延迟 | 0.0146 ms |
| 最大绝对误差 | 0.000e+00 |
| 验收 gate | true |

### 如何解释结果

教学版 Triton GEMM 达到 11.72 TFLOP/s，torch.mm 为 18.44；库函数应始终保留为基线。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/13-matmul-tiling/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。一个方阵 shape 不能证明在
skinny、batched、小矩阵和非对齐矩阵上都追平 cuBLAS。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
