...
# 第 30 课 — 从慢子图到可交付 kernel

> **问题：**当baseline、融合、验证、性能 gate 与回滚同时变化时，怎样判断原因来自 kernel、布局、工具链还是硬件边界？

[← 第 05 章](../README.md) · [English lesson](../../../chapters/05-triton-gpu-programming/30-deliverable-kernel/README.md) · [实验 Notebook](../../../chapters/05-triton-gpu-programming/30-deliverable-kernel/lab.ipynb) · [RTX 5090 结果](../../../chapters/05-triton-gpu-programming/30-deliverable-kernel/artifacts/rtx5090-result.json)

## 为什么值得研究

本课只隔离研究**baseline、融合、验证、性能 gate 与回滚**。目的不是把每个 PyTorch 操作都改成自定义代码，
...
可交付 kernel 不只是能运行的代码。它需要固定 baseline、明确输入 contract、极端正确性测试、延迟分布、环境身份、停止条件、集成路径与回滚方案；达到声明 gate
后就应停止优化。
...
```mermaid
flowchart LR
  A["固定输入与 contract"] --> B["baseline、融合、验证、性能 gate 与回滚"]
...
**实验：**交付 tanh 近似 GELU Triton kernel，并固定正确性阈值、重复计时、最低性能 gate 与 PyTorch rollback。
...
| 测量字段 | 仓库记录值 |
|---|---:|
| PyTorch/Triton 速度比 | 0.554x |
| Triton 中位延迟 | 0.0198 ms |
| 最大绝对误差 | 4.768e-07 |
| 验收 gate | false |

### 如何解释结果

可交付 GELU 相对 PyTorch baseline 为 0.55 倍，最大误差 4.77e-07；固定的正确性与非灾难回归 gate 判定为 False。

表格刻意只保留最关键字段。完整计时样本、target 身份、辅助字节或 shape 字段，以及 验收结果都在
[`rtx5090-result.json`](../../../chapters/05-triton-gpu-programming/30-deliverable-kernel/artifacts/rtx5090-result.json)
...
如果部署 shape、dtype、stride、编译器版本、目标架构、并发或周边 graph 改变，本结论 就可能失效。没有停止条件，kernel
优化会无限延伸，集成风险和维护成本却持续增长。出现这些变化，或 profiler 与预期机制冲突时，应重新打开 决策，而不是沿用旧数字。
