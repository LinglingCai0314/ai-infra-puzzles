...
# Lesson 24 — Backend Portability and the ROCm Boundary
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/24-backend-portability/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **shared source, backend targets, architecture tuning, and unmeasured
platforms**. The goal is not to turn every PyTorch operation into custom code. It is to make one
...
Triton's programming model can target more than one backend, but source portability is only the
first layer. Supported operations, code quality, tuning parameters, profiler tools, and
numerical behavior can still differ by architecture.
...
A kernel compiled on NVIDIA is not ROCm performance evidence, even if the source contains no
CUDA-specific call.
...
This run compiled the same Triton source for backend=cuda, arch=120. ROCm portability remains
unmeasured on this NVIDIA-only execution.
...
architecture, concurrency, or surrounding graph differs. A kernel compiled on NVIDIA is not ROCm
performance evidence, even if the source contains no CUDA-specific call. Reopen the decision
