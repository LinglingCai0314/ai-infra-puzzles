...
# Lesson 14 — Autotune Search and Experiment Budget
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/14-autotune-budget/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **configuration sets, cache keys, and search cost**. The goal is not to
...
triton.autotune benchmarks eligible configurations when a key changes and caches the winner.
This turns meta-parameter selection into measured search, but the first call now includes
multiple compilations and executions.
...
An autotuned kernel that mutates its output may need a reset hook because every candidate
...
Three configurations were eligible. The first host call cost 570.64 ms, while the cached warm
median was 0.0333 ms.
...
architecture, concurrency, or surrounding graph differs. An autotuned kernel that mutates its
output may need a reset hook because every candidate executes during search. Reopen the decision
