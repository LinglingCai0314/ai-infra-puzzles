...
# Lesson 25 — Dynamic Shapes and Specialization
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/25-dynamic-shapes/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **runtime arguments, constexpr meta-parameters, tails, and cache keys**.
...
A size used only in masks and grid calculation can remain a runtime argument, allowing one
compiled kernel to cover many lengths. Values that shape tl.arange or control compile-time
branches must be constexpr and create specialization boundaries.
...
Marking every shape value constexpr can create compilation churn; making every choice dynamic
can prevent useful optimization.
...
One runtime-size kernel handled 3 shapes and three different tails without changing source; the
largest warm median was 0.0193 ms.
...
architecture, concurrency, or surrounding graph differs. Marking every shape value constexpr can
create compilation churn; making every choice dynamic can prevent useful optimization. Reopen
