...
# Lesson 02 — Blocked Programs versus CUDA Threads
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/02-programming-models/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **program_id, blocked tensors, and scalar threads**. The goal is not to
...
CUDA exposes scalar threads grouped into blocks. Triton exposes a grid of program instances;
each instance evaluates tensor expressions such as tl.arange. The mapping can resemble one CUDA
block, but it is a reasoning aid rather than an identity.
...
Treating each lane of a blocked tensor as an independently scheduled CUDA thread leads to wrong
assumptions about synchronization and storage.
...
16,384 blocked programs covered 4,194,304 scalar elements; correctness matched torch.add and the
two timings are reported separately.
...
architecture, concurrency, or surrounding graph differs. Treating each lane of a blocked tensor
as an independently scheduled CUDA thread leads to wrong assumptions about synchronization and
storage. Reopen the decision when any of those conditions changes or when a profiler contradicts
