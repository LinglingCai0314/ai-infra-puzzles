...
# Lesson 04 — A Tail-Safe Vector Kernel
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/04-first-vector-kernel/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **program_id, tl.arange, mask, and BLOCK**. The goal is not to turn every
...
A vector kernel derives offsets from program_id and a compile-time BLOCK. The runtime element
count controls a tail mask, while BLOCK shapes the compiler-visible tensor. Correctness at
non-divisible lengths is part of the kernel contract.
...
Omitting the load and store mask may appear correct on aligned examples while reading or writing
beyond the allocation on the first awkward shape.
...
The odd-length tail remained correct. BLOCK=512 was fastest in this four-point sweep at 0.0187
ms.
...
architecture, concurrency, or surrounding graph differs. Omitting the load and store mask may
appear correct on aligned examples while reading or writing beyond the allocation on the first
awkward shape. Reopen the decision when any of those conditions changes or when a profiler
contradicts the proposed mechanism.
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 4 --end 4
