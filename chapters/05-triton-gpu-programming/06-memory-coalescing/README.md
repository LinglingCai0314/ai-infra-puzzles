...
# Lesson 06 — Diagnosing Coalescing with Strides
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/06-memory-coalescing/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **offset geometry, transactions, and requested bandwidth**. The goal is not
...
Coalescing follows the addresses requested by neighboring lanes, not the visual shape of a
tensor. The same row-copy source can produce adjacent or strided addresses when its column
stride changes. Requested bandwidth is useful only with the stride and byte formula beside it.
...
Adding shared memory before proving an access problem can increase work without fixing the
actual address pattern.
...
The same copy kernel took 0.0173 ms on contiguous columns and 0.0178 ms at stride two, a 1.03x
ratio.
...
architecture, concurrency, or surrounding graph differs. Adding shared memory before proving an
access problem can increase work without fixing the actual address pattern. Reopen the decision
