...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/12-reduction-and-scan/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **tree reduction, prefix dependency, and neutral values**. The goal is not
...
Reduction collapses a row to one value; scan produces every prefix. Triton expresses both as
blocked tensor operations, but their dependency graphs and output traffic differ. Tail padding
must still use the correct identity.
...
A very large row kept in one program can increase register pressure and reduce active work even
when the source is concise.
...
The row reduction took 0.0162 ms and inclusive scan 0.0155 ms. Their communication structures
differ even with identical input shapes.
...
architecture, concurrency, or surrounding graph differs. A very large row kept in one program
can increase register pressure and reduce active work even when the source is concise. Reopen
