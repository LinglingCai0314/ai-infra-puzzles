...
# Lesson 05 — Explicit CUDA Control and Error Boundaries
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/05-explicit-cuda-control/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **thread blocks, synchronization, nvcc, and reviewable source**. The goal
is not to turn every PyTorch operation into custom code. It is to make one performance claim
small enough that correctness, timing, layout, compilation, and the comparison path can all be
...
CUDA C++ makes the scalar thread index, block geometry, launch, synchronization, and error
checks explicit. Triton moves many of those choices into a blocked program and compiler. A fair
comparison needs both implementations and a toolchain that can actually build the CUDA path.
...
Source code availability is not native execution evidence. Without nvcc, a CUDA latency cell
...
|---|---|
| Baseline | reviewed CUDA C++ source plus toolchain availability |
...
The Triton path ran correctly in 0.0212 ms. nvcc availability was False, so the explicit CUDA
source is reviewable but is not presented as a native measurement.
...
architecture, concurrency, or surrounding graph differs. Source code availability is not native
execution evidence. Without nvcc, a CUDA latency cell must remain unmeasured. Reopen the
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 5 --end 5
