...
# Lesson 30 — From Slow Subgraph to Deliverable Kernel
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/30-deliverable-kernel/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **baseline, fusion, validation, performance gate, and rollback**. The goal
...
A deliverable kernel is more than code that runs. It has a frozen baseline, explicit input
contract, adversarial correctness suite, latency distribution, environment identity, stop
condition, integration path, and rollback. Optimization ends when the declared gate is met.
...
Without a stop condition, kernel work can continue indefinitely while integration risk and
maintenance cost grow.
...
The deliverable GELU reached 0.55x of the PyTorch baseline with 4.77e-07 max error, so its
frozen correctness and no-catastrophic-regression gate was False.
...
architecture, concurrency, or surrounding graph differs. Without a stop condition, kernel work
can continue indefinitely while integration risk and maintenance cost grow. Reopen the decision
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 30 --end 30
