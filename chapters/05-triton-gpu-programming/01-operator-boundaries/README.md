...
# Lesson 01 — Operator Boundaries and the Cost of Small Kernels
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/01-operator-boundaries/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **fusion, launch count, and memory traffic**. The goal is not to turn every
PyTorch operation into custom code. It is to make one performance claim small enough that
correctness, timing, layout, compilation, and the comparison path can all be inspected. The
...
An eager expression may read and write a full tensor at every operator boundary. A Triton
program can keep the intermediate value in registers and store once. The useful comparison
therefore freezes the algebra and separates launch count, requested bytes, and latency.
...
A faster fused kernel does not prove every operator should be rewritten. Very small inputs,
compiler fusion, or a strong library kernel can erase the advantage.
...
The fused Triton affine kernel took 0.0211 ms versus 0.0189 ms for the two-operation eager path,
with max error 4.77e-07.
...
architecture, concurrency, or surrounding graph differs. A faster fused kernel does not prove
every operator should be rewritten. Very small inputs, compiler fusion, or a strong library
kernel can erase the advantage. Reopen the decision when any of those conditions changes or when
a profiler contradicts the proposed mechanism.
...
python3 scripts/execute_chapter_notebooks.py --chapter 05 --start 1 --end 1
