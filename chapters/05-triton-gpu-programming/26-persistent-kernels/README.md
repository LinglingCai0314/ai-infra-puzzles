...
# Lesson 26 — Persistent Scheduling and the TMA Boundary
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/26-persistent-kernels/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **fixed resident programs, tile loops, launch geometry, and architecture
features**. The goal is not to turn every PyTorch operation into custom code. It is to make one
...
A persistent kernel launches fewer programs and lets each program loop over multiple tiles. This
can amortize scheduling or support producer-consumer pipelines, but it may reduce flexibility
and fairness. TMA is a separate architecture-specific data-movement mechanism, not a synonym for
persistence.
...
Labeling a tile loop 'TMA optimized' without a descriptor, supported load/store path, or
profiler evidence is overclaiming.
...
Persistent scheduling reduced the grid from 16,384 to 256 programs and took 0.0216 ms versus
0.0202 ms for the standard grid. TMA was not used.
...
architecture, concurrency, or surrounding graph differs. Labeling a tile loop 'TMA optimized'
without a descriptor, supported load/store path, or profiler evidence is overclaiming. Reopen
