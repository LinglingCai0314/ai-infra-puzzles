...
# Lesson 07 — Masks and Reduction Identities
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/07-mask-semantics/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **masked loads, other values, and reduction algebra**. The goal is not to
...
A mask does more than prevent an invalid address: the value supplied for an inactive lane enters
subsequent tensor algebra. Sum needs zero, max needs negative infinity, and min needs positive
infinity. The correct identity depends on the downstream operation.
...
Padding an all-negative max reduction with zero silently returns a plausible but impossible
answer.
...
Using -inf as the masked max identity produced 0.00e+00 error; padding with zero corrupted 1024
all-negative rows.
...
architecture, concurrency, or surrounding graph differs. Padding an all-negative max reduction
with zero silently returns a plausible but impossible answer. Reopen the decision when any of
