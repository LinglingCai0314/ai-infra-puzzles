...
# Lesson 22 — PyTorch Integration with torch.compile
...
[← Chapter 05](../README.md) · [中文本课](../../../chapters-zh/05-triton-gpu-programming/22-torch-compile-integration/README.md) · [Executed notebook](lab.ipynb) · [RTX 5090 result](artifacts/rtx5090-result.json)

## Why this puzzle matters

This lesson isolates **graph capture, Inductor, generated kernels, and cold start**. The goal is
...
torch.compile captures a PyTorch graph and lets Inductor generate fused code, often using Triton
on CUDA. This route can remove operator boundaries without maintaining a manual kernel, but
compilation, graph breaks, guards, and dynamic shapes become part of the system.
...
Comparing compiled cold start with eager warm time, or attributing all compiled speedup to one
generated kernel, produces an invalid conclusion.
...
| Candidate | optimized framework path described in the experiment |
...
torch.compile cold execution took 1013.72 ms; its warm median was 0.0557 ms versus 0.0261 ms
eager. Compile cost and steady state stay separate.
...
architecture, concurrency, or surrounding graph differs. Comparing compiled cold start with
eager warm time, or attributing all compiled speedup to one generated kernel, produces an
invalid conclusion. Reopen the decision when any of those conditions changes or when a profiler
